...
create table bank1(Account_No int primary key auto_increment, 
Acc_Name varchar(30) not null, Balance numeric(10,2));
...
insert into bank1(Acc_Name, Balance ) values('Archana',50000.00);
...
BEGIN
    SELECT sum(Transaction_amt) INTO WTotal FROM assignment.bank_udpate1
	WHERE Actions = 'Debit' AND Account_No=Account_No AND changed_id >= Date_sub(now(),interval 1 hour);
    
    SELECT sum(Transaction_amt) INTO DTotal FROM assignment.bank_udpate1
...
-- DROP THR EVENT --Stored Procedures
DROP EVENT MyEvent;
